 ...
 */
public final class DarkBlue extends JFrame implements MouseListener, ActionListener{
	...
	*/
	private final void AddComponentsToPane(final Container a_pane){
		...
		m_bottom.setBackground(Color.BLUE);
		...
		for(int index = Utilities.ZERO; index < activePiecesCopy.size(); index++){
			...
			if(activePiecesCopy.get(index).IsPawn()){
				if((a_player.IsWhite() && activePiecesCopy.get(index).GetCurrentRow() == Utilities.ZERO) || (a_player.IsBlack() && activePiecesCopy.get(index).GetCurrentRow() == Utilities.SEVEN)){
					...
					m_board.SetBoard(pawn.Promote(m_board.GetBoard()));
					return;
				}
			}else{
				continue;
	...
	*/
	private final void TryPlay(){
		try{
			...
			JOptionPane.showMessageDialog(null, "Dark Blue has quit unexpectedly.\n" + e.getMessage(), FATAL_ERROR_TITLE, JOptionPane.ERROR_MESSAGE);
	...
	*/
	private final void ChooseColor(){
		...
		while(true){
			...
			m_board = GUIBoard.GetEnPassantTest();// Originally GUIBoard.GetStartingPosition()
